...
def render_report(report: SyncReport, generated_at: datetime) -> str:
    ...
    lines.append(f"# Sync report — {title_at}")
    lines.append("")
    lines.append("## Summary")
    lines.append("")
    lines.append(f"- Upstream HEAD: `{report.upstream_head}` ({UPSTREAM_REPO})")
    lines.append(f"- Files inspected: {report.files_inspected}")
    lines.append(f"- Files translated: {len(report.entries)}")
    lines.append(f"- Files skipped: {len(report.skipped)}")
    lines.append(f"- Files failed: {len(report.failures)}")
    lines.append(f"- Model: `{report.model}` via `{report.base_url}`")
    ...
    lines.append("---")
    lines.append("")
    lines.append("Auto-generated by `scripts/sync_descriptions.py`. "
                 "Please review the Chinese translations before merging.")
    lines.append("")
    return "\n".join(lines)
...
def write_report(report: SyncReport, dry_run: bool) -> Optional[Path]:
    ...
    if dry_run:
        print(f"DRY_RUN: would write report to {out_path} "
              f"({len(content)} bytes, {len(report.entries)} translation(s))")
        return out_path
